...
final class ResetPasswordRequestFixtures extends Fixture implements \Doctrine\Common\DataFixtures\DependentFixtureInterface
{
    private const ENTITY_CLASS = ResetPasswordRequest::class;
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        for ($index = 1; $index <= $this->recordCount(); ++$index) {
            $entity = new ResetPasswordRequest($this->getReference($this->reference(User::class, $index), User::class), new \DateTimeImmutable('+1 day'), sprintf('reset-%06d', $index), hash('sha256', sprintf('reset-token-%06d', $index)));
