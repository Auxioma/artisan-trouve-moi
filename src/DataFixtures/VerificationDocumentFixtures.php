...
final class VerificationDocumentFixtures extends Fixture implements \Doctrine\Common\DataFixtures\DependentFixtureInterface
{
    private const ENTITY_CLASS = VerificationDocument::class;
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        for ($index = 1; $index <= $this->recordCount(); ++$index) {
            ...
            $directory = $this->kernel->getProjectDir().DIRECTORY_SEPARATOR.'var'.DIRECTORY_SEPARATOR.'fixture-uploads'.DIRECTORY_SEPARATOR.'VerificationDocument';
            if (!is_dir($directory)) { mkdir($directory, 0775, true); }
            $copy = $directory.DIRECTORY_SEPARATOR.sprintf('%06d-worksite.jpg', $index);
            copy($source, $copy);
            $entity->setDocumentFile(new UploadedFile($copy, basename($source), 'image/jpeg', null, true));
